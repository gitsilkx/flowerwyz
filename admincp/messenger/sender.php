...
/*
  ...
  $Id: sender.php 107 2007-03-25 19:49:18Z matt.simpson $
*/
...
// Setup PHP and start page setup.
  ...
  switch($_GET["action"]) {
    ...
    case "resume" :
      ...
      echo "\twindow.location='sender.php?sid=".session_id()."&qid=".(int) $_SESSION["queue_details"]["queue_id"]."&action=send';";
      echo "</script>\n";
      @flush();
      @ob_flush();
    break;
      ...
      if(isset($_SESSION["message_details"])) {
        ...
        if($results) {
          foreach($results as $result) {
            $progress++;
            if($result["sent"] == "0") {
              if($user_data = get_custom_data($result["users_id"], array("messageid" => (((isset($_SESSION["message_details"]["message_id"])) && ((int) $_SESSION["message_details"]["message_id"])) ? $_SESSION["message_details"]["message_id"] : 0)))) {
          ...
          if($db->Execute($query)) {
            ...
            if($sent_batch != $total_batches) {
              ...
              echo "\tsetTimeout('window.location=\'sender.php?sid=".session_id()."&qid=".$_SESSION["queue_details"]["queue_id"]."&action=send\'', ".($_SESSION["config"][PREF_PAUSE_BETWEEN] * 1000).");";
              echo "</script>\n";
            } else {
              echo "<script language=\"JavaScript\" type=\"text/javascript\">\n";
              echo "\tparent.document.getElementById('buttonHTML').innerHTML  = '';\n";
              echo "\tparent.document.getElementById('progressText').innerHTML = 'Sent batch ".$sent_batch." of ".$total_batches.".';\n";
              echo "\twindow.location = 'sender.php?sid=".session_id()."&qid=".$_SESSION["queue_details"]["queue_id"]."&action=send';";
              echo "</script>\n";
            }
          } else {
            if($_SESSION["config"][PREF_ERROR_LOGGING] == "yes") {
    ...
    default : // Queue Messages.
      ...
      if((@is_array($_SESSION["queue_details"]["target"])) && (@count($_SESSION["queue_details"]["target"]) > 0)) {
        ...
        if(!$db->Execute($query)) {
          ...
          echo "\t\treturn;\n";
          echo "\t} else {\n";
          echo "\t\twindow.location='sender.php?sid=".session_id()."&qid=".(int) $_SESSION["queue_details"]["queue_id"]."&action=cancel';\n";
          echo "\t\treturn;\n";
          echo "\t}\n";
          echo "}\n\n";
