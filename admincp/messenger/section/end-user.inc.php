...
/*
	...
	$Id: end-user.inc.php 107 2007-03-25 19:49:18Z matt.simpson $
*/
...
$i = count($SIDEBAR);
$SIDEBAR[$i]  = "<h1>End-User Systems</h1>\n";
if($_SESSION["config"][ENDUSER_ARCHIVE] != "no") {
	...
	$SIDEBAR[$i] .= "</div>\n";
	$SIDEBAR[$i] .= "<div>";
	$SIDEBAR[$i] .= "	<img src=\"./images/icon-checkmark.gif\" width=\"20\" height=\"20\" alt=\"RSS Feed Online\" title=\"RSS Feed Online\" style=\"vertical-align: middle\" /> <a href=\"".$_SESSION["config"][PREF_PUBLIC_URL].$_SESSION["config"][ENDUSER_ARCHIVE_FILENAME]."?view=rss\" style=\"vertical-align: middle\" target=\"_blank\">RSS Feed Online</a>\n";
	$SIDEBAR[$i] .= "</div>\n";
} else {
	$SIDEBAR[$i] .= "<div>";
...
$SIDEBAR[$i] .= "If you would like to change the state of these systems, visit the <a href=\"./index.php?section=preferences&type=enduser\" style=\"font-weight: normal; font-size: 11px\">End-User Preferences</a> section.\n";
$SIDEBAR[$i] .= "</div>\n";
...
<ol>
	...
	<li style="padding-bottom: 10px">
		...
		Here are several examples of subscription forms that you can use to add subscribers to your mailing list from your website. Please keep in mind that these are examples and will require you to enter the corresponding group ID that you would like the subscribers added to. If you go to the Manage Groups sections within ListMessenger you will see that each group has Group ID displayed to the right of the group name. Please remember to replace <em>ENTER_GROUP_ID_HERE</em> with the actual ID of the group or groups you wish to allow your subscribers to subscribe to.
		<br /><br />
		<div style="border-bottom: 1px #CCCCCC solid">Example <strong>One</strong>: All custom fields included.</div>
		<br />
		<div class="tab-pane" id="tab-pane-example1">
			<div class="tab-page" style="height: 100%">
				<h2 class="tab">Preview</h2>
				<?php echo generate_cfields($_SESSION["config"][PREF_PUBLIC_URL].$_SESSION["config"][ENDUSER_FILENAME]); ?>
			</div>
			<div class="tab-page" style="height: 100%">
				<h2 class="tab">Source Code</h2>
				<form>
...
<!-- End of example one. (To here) -->
		...
		foreach($examples as $number => $example) {
			...
			echo "<br />\n";
			echo "<div class=\"tab-pane\" id=\"tab-pane-example-".$number."\">\n";
			echo "	<div class=\"tab-page\" style=\"height: 100%\">\n";
			echo "		<h2 class=\"tab\">Preview</h2>\n";
			...
			echo "	</div>\n";
			echo "	<div class=\"tab-page\" style=\"height: 100%\">\n";
			echo "		<h2 class=\"tab\">Source Code</h2>\n";
			echo "		<form>\n";
	...
	<li style="padding-bottom: 10px">
		...
		<ul>
			...
			<li style="padding-bottom: 5px">
				...
				This directory contains all of the language files for the end-user tools. If you would like to translate the end-user tools, simply copy the default english.lang.php file to yourlanguage.lang.php and make the changes within. When you're finished, place your new language file in this directory and select it from the drop down in the End-User Preferences within the Control Panel.
			</li>
			<li style="padding-bottom: 5px">
