...
/*
  ...
  $Id: subscribers.inc.php 107 2007-03-25 19:49:18Z matt.simpson $
*/
...
switch($ACTION) {
  case "add" :
    if($_POST) {
      if(strlen(trim($_POST["email_address"])) < 1) {
        ...
        if($results) {
          foreach($results as $result) {
            if(!$_POST["cdata"][$result["field_sname"]]) {
              $ERROR++;
              $ERRORSTR[] = "The custom field &quot;".$result["field_lname"]."&quot; has been set to be a required field.";
            }
          }
        }
      }
      if(!$ERROR) {
        ...
        if($_POST["confirmation"] == "1") {
          $result = users_queue(trim($_POST["email_address"]), trim($_POST["firstname"]), trim($_POST["lastname"]), $groups, $_POST["cdata"], "adm-subscribe");
          if($result) {
          ...
          $result = users_add(trim($_POST["email_address"]), trim($_POST["firstname"]), trim($_POST["lastname"]), $groups, $_POST["cdata"], "private");
          if($result) {
    ...
    if(groups_count(0, $groups) < 1) {
      ...
      <tbody>
        <tr>
          <td><label for="email_address" class="form-row-req">E-Mail Address:</label></td>
        ...
        <tr>
          <td><label for="firstname" class="form-row-nreq">Firstname:</label></td>
        ...
        <tr>
          <td><label for="lastname" class="form-row-nreq">Lastname:</td>
        ...
        <tr>
          <td style="vertical-align: top"><label for="group_ids" class="form-row-req">Subscriber Group:</label></td>
          <td>
        ...
        if($results) {
          foreach($results as $result) {
            echo "<tr>\n";
            echo "  <td style=\"vertical-align: top\" class=\"".(((int) $result["field_req"]) ? "form-row-req" : "form-row-nreq")."\">".(($result["field_lname"]) ? html_encode(checkslashes($result["field_lname"], 1)) : "&nbsp;")."</td>\n";
            echo "  <td>\n";
            switch($result["field_type"]) {
              case "checkbox" :
                if($result["field_options"]) {
                  ...
                  foreach($options as $option) {
                    ...
                    echo "<input type=\"checkbox\" name=\"cdata[".$result["field_sname"]."][]\" value=\"".html_encode($pieces[0])."\"".((@in_array($pieces[0], $_POST["cdata"][$result["field_sname"]])) ? " checked=\"checked\"" : "")." onkeypress=\"return handleEnter(this, event)\" /> ".html_encode($pieces[1])."<br />\n";
                  }
                } else {
              ...
              case "radio" :
                if($result["field_options"]) {
                  ...
                  foreach($options as $option) {
                    $pieces = explode("=", $option);
                    echo "<input type=\"radio\" name=\"cdata[".$result["field_sname"]."]\" value=\"".html_encode($pieces[0])."\"".(($_POST["cdata"][$result["field_sname"]] == $pieces[0]) ? " checked=\"checked\"" : "")." onkeypress=\"return handleEnter(this, event)\" /> ".html_encode($pieces[1])."<br />\n";
                  }
                } else {
              ...
              case "select" :
                if($result["field_options"]) {
                  $options = explode("\n", $result["field_options"]);
                  echo "<select name=\"cdata[".$result["field_sname"]."]\" onkeypress=\"return handleEnter(this, event)\">\n";
                  foreach($options as $option) {
                    $pieces = explode("=", $option);
              ...
              break;
              case "textarea" :
                echo "<textarea name=\"cdata[".$result["field_sname"]."]\" style=\"width: 98%; height: 75px\">".((trim($_POST["cdata"][$result["field_sname"]])) ? html_encode($_POST["cdata"][$result["field_sname"]]) : "")."</textarea>\n";
              break;
              case "textbox" :
                echo "<input type=\"text\" name=\"cdata[".$result["field_sname"]."]\" value=\"".((trim($_POST["cdata"][$result["field_sname"]])) ? html_encode($_POST["cdata"][$result["field_sname"]]) : "")."\"".((strlen($result["field_length"]) > 0) ? " maxlength=\"".$result["field_length"]."\"" : "")." onkeypress=\"return handleEnter(this, event)\" />\n";
              break;
              default :
                echo "&nbsp;";
              break;
                ...
                <td style="padding-bottom: 10px">
                  ...
                  <thead>
                    <tr>
                      ...
                      <td style="height: 15px; padding-left: 2px; border-top: 1px #9D9D9D solid; border-left: 1px #9D9D9D solid; border-bottom: 1px #9D9D9D solid; background-image: url('./images/table-head-off.gif'); white-space: nowrap"><span class="theading-off">Name Field</span></td>
                      <td style="height: 15px; padding-left: 2px; border-top: 1px #9D9D9D solid; border-left: 1px #9D9D9D solid; border-bottom: 1px #9D9D9D solid; background-image: url('./images/table-head-off.gif'); white-space: nowrap"><span class="theading-off">E-Mail Address</span></td>
                      <td style="height: 15px; padding-left: 2px; border: 1px #9D9D9D solid; background-image: url('./images/table-head-off.gif'); white-space: nowrap"><span class="theading-off">Group</span></td>
                    </tr>
                  </thead>
                  <tbody>
                  <?php
  ...
  case "delete" :
    if((isset($_POST["subscribers"])) && (is_array($_POST["subscribers"])) && (count($_POST["subscribers"]))) {
      if((!isset($_POST["confirmed"])) || (!(int) $_POST["confirmed"])) {
        ...
        Please review the list of selected subscribers below. The subscribers who's checkbox is checked will be permanently removed from the database after you click the &quot;Confirmed&quot; button.
        <br /><br />
        ...
        <tr>
          ...
          <td style="width: 25%; height: 15px; padding-left: 2px; border-top: 1px #9D9D9D solid; border-left: 1px #9D9D9D solid; border-bottom: 1px #9D9D9D solid; background-image: url('./images/table-head-off.gif'); white-space: nowrap"><span class="theading-off">Name Field</span></td>
          <td style="width: 33%; height: 15px; padding-left: 2px; border-top: 1px #9D9D9D solid; border-left: 1px #9D9D9D solid; border-bottom: 1px #9D9D9D solid; background-image: url('./images/table-head-off.gif'); white-space: nowrap"><span class="theading-off">E-Mail Address</span></td>
          <td style="width: 20%; height: 15px; padding-left: 2px; border-top: 1px #9D9D9D solid; border-left: 1px #9D9D9D solid; border-bottom: 1px #9D9D9D solid; background-image: url('./images/table-head-off.gif'); white-space: nowrap"><span class="theading-off">Signup Date</span></td>
          <td style="width: 19%; height: 15px; padding-left: 2px; border: 1px #9D9D9D solid; background-image: url('./images/table-head-off.gif'); white-space: nowrap"><span class="theading-off">Group</span></td>
        </tr>
        <?php
        foreach($_POST["subscribers"] as $users_id) {
          ...
          if($result) {
            ...
            echo "  <td>".(($result["name_field"] != ", ") ? html_encode(limit_chars(((substr($result["name_field"], -2) == ", ") ? substr($result["name_field"], 0, (strlen($result["name_field"])-2)) : $result["name_field"]), 20)) : "&nbsp;")."</td>\n";
            echo "  <td>".html_encode(limit_chars($result["email_address"], 28))."</td>\n";
            echo "  <td>".display_date("m/d/Y H:i", $result["signup_date"])."</td>\n";
            echo "  <td>".html_encode(limit_chars($result["group_name"], 25))."</td>\n";
            echo "</tr>\n";
          } else {
            echo "<tr style=\"background-color: #FFD9D0\">\n";
            echo "  <td colspan=\"5\" style=\"height: 20px; padding-left: 5px\">User ID ".$users_id." was not found.</td>\n";
            echo "</tr>\n";
          }
        }
        echo "<tr>\n";
        echo "  <td colspan=\"5\" style=\"border-top: 1px #333333 dotted; padding-top: 5px; text-align: right\"><input type=\"button\" value=\"Cancel\" class=\"button\" onclick=\"window.location='index.php'\" />&nbsp;<input type=\"submit\" value=\"Confirmed\" class=\"button\" /></td>\n";
        echo "</tr>\n";
        echo "</table>\n";
        echo "</form>\n";
      } else {
  ...
  case "edit" :
      ...
      if(!isset($_POST["email_address"])) {
        ...
        if($results) {
          foreach($results as $result) {
            if(!$_POST["cdata"][$result["field_sname"]]) {
              $ERROR++;
              $ERRORSTR[] = "The custom field &quot;".html_encode($result["field_lname"])."&quot; has been set to be a required field.";
            }
          }
        }
      }
            ...
            foreach($results as $result) {
              ...
              if(!$ERROR) {
                ...
                if($db->Execute($squery)) {
                  if((@is_array($_POST["cdata"])) && (@count($_POST["cdata"]) > 0)) {
                    foreach($_POST["cdata"] as $field_sname => $value) {
                      $cfield_id  = get_field_id($field_sname);
                      if($cfield_id) {
                        $squery  = "SELECT `cdata_id` FROM `".TABLES_PREFIX."cdata` WHERE `cfield_id`='".$cfield_id."' AND `user_id`='".$users_id."'";
                        $sresult  = $db->GetRow($squery);
                        if($sresult) {
                          $squery  = "UPDATE `".TABLES_PREFIX."cdata` SET `value`='".((@is_array($value)) ? implode(", ", $value) : checkslashes(trim($value)))."' WHERE `cdata_id`='".$sresult["cdata_id"]."'";
                          if(!$db->Execute($squery)) {
                            if($_SESSION["config"][PREF_ERROR_LOGGING] == "yes") {
                          ...
                          $squery  = "INSERT INTO `".TABLES_PREFIX."cdata` VALUES (NULL, '".$users_id."', '".$cfield_id."', '".((@is_array($value)) ? implode(", ", $value) : checkslashes(trim($value)))."');";
                          if(!$db->Execute($squery)) {
                            if($_SESSION["config"][PREF_ERROR_LOGGING] == "yes") {
            ...
            if($db->Execute($query)) {
              if((@is_array($_POST["cdata"])) && (@count($_POST["cdata"]) > 0)) {
                foreach($_POST["cdata"] as $field_sname => $value) {
                  $cfield_id  = get_field_id($field_sname);
                  if($cfield_id) {
                    $query  = "SELECT `cdata_id` FROM `".TABLES_PREFIX."cdata` WHERE `cfield_id`='".$cfield_id."' AND `user_id`='".checkslashes(trim($_GET["id"]))."'";
                    $result  = $db->GetRow($query);
                    if($result) {
                      $query  = "UPDATE `".TABLES_PREFIX."cdata` SET `value`='".((@is_array($value)) ? implode(", ", $value) : checkslashes(trim($value)))."' WHERE `cdata_id`='".$result["cdata_id"]."'";
                      if(!$db->Execute($query)) {
                        if($_SESSION["config"][PREF_ERROR_LOGGING] == "yes") {
                          @error_log(display_date("r", time())."\t".__FILE__." [Line: ".__LINE__."]\tUnable to update custom field data in the custom field table. Database server said: ".$db->ErrorMsg()."\n", 3, $_SESSION["config"][PREF_PRIVATE_PATH]."logs/error_log.txt");
                        }
                      }
                    } else {
                      $query  = "INSERT INTO `".TABLES_PREFIX."cdata` VALUES (NULL, '".trim($_GET["id"])."', '".$cfield_id."', '".((@is_array($value)) ? implode(", ", $value) : checkslashes(trim($value)))."');";
                      if(!$db->Execute($query)) {
                        if($_SESSION["config"][PREF_ERROR_LOGGING] == "yes") {
    ...
    if($result) {
      ...
      <tbody>
        ...
        <tr>
          <td><label for="lastname" class="form-row-nreq">Lastname:</label></td>
        ...
        <tr>
          <td style="vertical-align: top"><label for="group_id" class="form-row-req">Subscriber Group:</label></td>
          <td>
        ...
        if($results) {
          foreach($results as $result) {
            ...
            echo "<tr>\n";
            echo "  <td style=\"vertical-align: top\" class=\"".(($result["field_req"] == 1) ? "form-row-req" : "form-row-nreq")."\">".(($result["field_lname"]) ? html_encode(checkslashes($result["field_lname"], 1)) : "&nbsp;")."</td>\n";
            echo "  <td>\n";
            switch($result["field_type"]) {
              case "checkbox" :
                if($result["field_options"]) {
        ...
        <tr>
          <td style="padding: 10px">
            ...
            if($sresults) {
              ...
              <table style="width: 100%; text-align: left" cellspacing="0" cellpadding="1" border="0">
              <tr>
                <td style="width: 3%; height: 15px; padding-left: 2px; border-top: 1px #9D9D9D solid; border-left: 1px #9D9D9D solid; border-bottom: 1px #9D9D9D solid; background-image: url('./images/table-head-off.gif')">&nbsp;</td>
                <td style="width: 20%; height: 15px; padding-left: 2px; border-top: 1px #9D9D9D solid; border-left: 1px #9D9D9D solid; border-bottom: 1px #9D9D9D solid; background-image: url('./images/table-head-on.gif')">Date</td>
                <td style="width: 37%; height: 15px; padding-left: 2px; border-top: 1px #9D9D9D solid; border-left: 1px #9D9D9D solid; border-bottom: 1px #9D9D9D solid; background-image: url('./images/table-head-off.gif')">Action</td>
                <td style="width: 25%; height: 15px; padding-left: 2px; border-top: 1px #9D9D9D solid; border-left: 1px #9D9D9D solid; border-bottom: 1px #9D9D9D solid; background-image: url('./images/table-head-off.gif')">IP Address</td>
                <td style="width: 15%; height: 15px; padding-left: 2px; border: 1px #9D9D9D solid; background-image: url('./images/table-head-off.gif')">Confirmed</td>
              </tr>
              <?php
              foreach($sresults as $sresult) {
                unset($client);
                ...
                echo "  <td class=\"cursor\">".display_date("m/d/Y H:i", $sresult["date"])."</td>\n";
  ...
  default :
    ...
    // Begin Query String
    if((isset($_GET["q"])) && (isset($_GET["t"])) && (isset($_GET["f"]))) {
      $search_querying["search"] = true;

      if(isset($_GET["f"])) {
        switch($_GET["f"]) {
          case "fname" :
            $search_query .= " WHERE `firstname`";
          break;
          case "lname" :
            $search_query .= " WHERE `lastname`";
          break;
          case "email" :
            $search_query .= " WHERE `email_address`";
          break;
          case "name" :
          default :
            $search_query .= " WHERE CONCAT_WS(', ', `lastname`, `firstname`)";
          break;
        }
      }
      ...
      if(isset($_GET["t"])) {
        switch($_GET["t"]) {
          case "equals" :
            $search_query .= " = '".checkslashes(trim($_GET["q"]))."'";
          break;
          case "contains" :
          default :
            $search_query .= " LIKE '%".checkslashes(trim($_GET["q"]))."%'";
          break;
        }
      }
    } elseif((int) trim($_GET["g"])) {
      $search_querying["group"] = true;

      $groups    = array();
      $groups[]  = (int) trim($_GET["g"]);
      groups_inarray((int) trim($_GET["g"]), $groups);

      $search_query .= " WHERE `group_id` IN ('".implode("', '", $groups)."')";
    }

    $query    = "SELECT COUNT(*) AS `totalrows` FROM `".TABLES_PREFIX."users`".$search_query;
    $result    = $db->GetRow($query);
    ...
    <div style="display: <?php echo (in_array("search", $COLLAPSED) ? "none" : "inline"); ?>" id="opened_search">
      ...
      <tr>
        <td>
          ...
          <tr>
            <td>
              <label for="f" class="form-row-nreq" style="vertical-align: middle">User Search:</label>
              <select id="f" name="f">
                <option value="name"<?php echo (($_GET["f"] == "name") ? " selected=\"selected\"" : ""); ?>>Name Field</option>
                <option value="fname"<?php echo (($_GET["f"] == "fname") ? " selected=\"selected\"" : ""); ?>>Firstname</option>
                <option value="lname"<?php echo (($_GET["f"] == "lname") ? " selected=\"selected\"" : ""); ?>>Lastname</option>
                <option value="email"<?php echo (($_GET["f"] == "email") ? " selected=\"selected\"" : ""); ?>>E-Mail Address</option>
              </select>
            </td>
            <td>
              <select name="t">
            ...
            <td>
              <input type="text" class="text-box" style="width: 165px" name="q" value="<?php echo html_encode(checkslashes($_GET["q"], 1)); ?>" />
            </td>
      ...
      <tr>
        <td>
          <form action="index.php" id="changegroup" method="get">
          <table style="width: 100%; height: 25px" cellspacing="0" cellpadding="2" border="0">
          <tr>
            <td>
              <label for="g" class="form-row-nreq" style="vertical-align: middle">Show Group:</label>
              <select id="g" name="g" style="width: 204px" onchange="document.getElementById('changegroup').submit();">
    ...
    if($results) {
      ...
      <tr>
        <td style="width: 50%; text-align: left">
          ...
          } elseif($search_querying["group"]) {
            echo "  <input type=\"hidden\" name=\"g\" value=\"".html_encode($_GET["g"])."\" />";
          }
          ?>
          ...
          if($totalpages > 1) {
            ...
            <tr>
              ...
              <td style="text-align: center">
                ...
                echo "<select name=\"vp\" onchange=\"document.getElementById('changepage').submit();\"".(($totalpages <= 1) ? " DISABLED" : "").">\n";
                if(!$totalpages) {
      ...
      <thead>
        <tr>
          <td style="height: 15px; padding-left: 2px; border-top: 1px #9D9D9D solid; border-left: 1px #9D9D9D solid; border-bottom: 1px #9D9D9D solid; background-image: url('./images/table-head-off.gif'); white-space: nowrap">&nbsp;</td>
          <td style="height: 15px; padding-left: 2px; border-top: 1px #9D9D9D solid; border-left: 1px #9D9D9D solid; border-bottom: 1px #9D9D9D solid; background-image: url('./images/table-head-<?php echo (($sort == "name") ? "on" : "off"); ?>.gif'); white-space: nowrap"><?php echo order_link("name", "Name Field", $order, $sort); ?></td>
          <td style="height: 15px; padding-left: 2px; border-top: 1px #9D9D9D solid; border-left: 1px #9D9D9D solid; border-bottom: 1px #9D9D9D solid; background-image: url('./images/table-head-<?php echo (($sort == "email") ? "on" : "off"); ?>.gif'); white-space: nowrap"><?php echo order_link("email", "E-Mail Address", $order, $sort); ?></td>
          <td style="height: 15px; padding-left: 2px; border-top: 1px #9D9D9D solid; border-left: 1px #9D9D9D solid; border-bottom: 1px #9D9D9D solid; background-image: url('./images/table-head-<?php echo (($sort == "date") ? "on" : "off"); ?>.gif'); white-space: nowrap"><?php echo order_link("date", "Signup Date", $order, $sort); ?></td>
          <td style="height: 15px; padding-left: 2px; border: 1px #9D9D9D solid; background-image: url('./images/table-head-<?php echo (($sort == "group") ? "on" : "off"); ?>.gif'); white-space: nowrap"><?php echo order_link("group", "Group", $order, $sort); ?></td>
        </tr>
      </thead>
      <tfoot>
        <tr>
          <td colspan="5" style="border-top: 1px #333333 dotted; padding-top: 5px">
            <input type="checkbox" name="selectall" value="1" onclick="selection(this.form['subscribers[]'])" style="vertical-align: middle" />&nbsp;
      ...
      <tbody>
        ...
        foreach($results as $result) {
          ...
          echo "  <td class=\"cursor\" onclick=\"window.location='index.php?section=subscribers&action=view&id=".$result["users_id"]."'\">".display_date("m/d/Y H:i", $result["signup_date"])."</td>\n";
      ...
      if((strlen($_GET["q"]) > 0) || (strlen($_GET["t"]) > 0) || (strlen($_GET["f"]) > 0) || (strlen($_GET["g"]) > 0)) {
        ...
        <div class="generic-message">
          There were no subscribers returned based on the search criteria you have providied.
          <br /><br />
        ...
        if(($result) && ($result["total_groups"])) {
          echo "Now that you have ".(($result["total_groups"] != 1) ? "some ListMessenger Groups" : "a ListMessenger Group")." created you might want to put an End-User subscriber form on your website so your visitors can subscribe to your group".(($result["total_groups"] != 1) ? "s" : "").". Examples of End-User subscriber forms that you can use are provided in the <a href=\"index.php?section=end-user\">End-User Tools</a> section within the Control Panel.\n";
          echo "<br /><br />\n";
          echo "If you would like to manually add subscribers to your group".(($result["total_groups"] != 1) ? "s" : "")." you can click the <a href=\"index.php?section=subscribers&amp;action=add\">Add Subscriber</a> link in the sidebar or use ListMessengers <a href=\"index.php?section=import-export&amp;action=import\">Import Mailing List</a> tools which are also available in the Control Panel.\n";
        } else {
