...
function static_product_cnt() {

    $result = mysql_query("SELECT COUNT(id) `prv_totalrow` FROM " . TABLE_PRODUCT." WHERE type_id=1"); //Count from local API dynamic data
    $row = mysql_fetch_array($result);
    return $row['prv_totalrow'];
    mysql_free_result($result);
}

function static_product_deactive_cnt() {

    $result = mysql_query("SELECT COUNT(id) `prv_totalrow` FROM " . TABLE_IMAGE." WHERE status='n'"); //Count from local API dynamic data
    $row = mysql_fetch_array($result);
    return $row['prv_totalrow'];
    mysql_free_result($result);
}

function static_product_active_cnt() {

    $result = mysql_query("SELECT COUNT(id) `prv_totalrow` FROM " . TABLE_IMAGE." WHERE status='y'"); //Count from local API dynamic data
    $row = mysql_fetch_array($result);
    return $row['prv_totalrow'];
    mysql_free_result($result);
}

function dynamic_product_cnt() {

$result = mysql_query("SELECT COUNT(id) `cnt` FROM " . TABLE_IMAGE." WHERE status='y'"); //Count from local static data
$row = mysql_fetch_array($result);
return $row['cnt'];
mysql_free_result($result);
}

function dynamic_delete_cnt(){
...
if (isset($ins->arrProducts) && count($ins->arrProducts) > 0):
    ...
    $i = count($ins->arrProducts);
 


    $message .="a) Count from foreign API data: " . $i . "<br>";
    $message .="b) Count from local API dynamic data: " . dynamic_product_cnt() . "<br>";
    $message .="c) Count from local static data: " . static_product_cnt() . "<br>";
    $message .="d) Count from local static [Deleted] data: " . static_delete_cnt() . "<br>";
    $message .="e) Count from local dynamic [Deleted] data: " . dynamic_delete_cnt() . "<br>";
    $message .="f) Count from local static [Added] data: " . static_add_cnt() . "<br>";
    $message .="g) Count from local dynamic [Added] data: " . dynamic_add_cnt() . "<br>";
    $message .="h) Mismatched volume since last Cron [a - b] = " . ($i - dynamic_product_cnt()) . "<br>";
    $message .="i) Mismatched volume since last Cron [a - c] = " . ($i - static_product_cnt()) . "<br><br>";
    ...
    mysql_query("DELETE FROM " . TABLE_PRODUCT." WHERE type_id=1"); //step 4
    ...
    $message12 .="i) Count from local API dynamic data: " . dynamic_product_cnt() . "<br>";
    $message12 .="i) Count from local Static data: " . static_product_cnt() . "<br>";
    $message12 .="j) Count from local dynamic [Deleted] data: " . dynamic_delete_cnt() . "<br>";
    $message12 .="k) Count from local dynamic [Added] data: " . dynamic_add_cnt() . "<br><br>";
    
    //print_r($ins->arrProducts);
    
    foreach ($ins->arrProducts as $product):
        if ($product['code'] <> '') {
            $r = mysql_query("insert into " . TABLE_PRODUCT . " set name='" . mysql_escape_string(trim($product['name'])) . "',product_no='" . $product['code'] . "',image='" . $product['image'] . "',description='" . mysql_escape_string(trim($product['description'])) . "',thumb_image='" . $product['thumbnail'] . "',price='" . $product['price'] . "',category='" . strtolower($product['productType']) . "',type_id='1'");  //step 5
            if (!in_array($product['code'], $code)) {
                $r = mysql_query("insert into " . TABLE_ADD_PRODUCT . " set name='" . mysql_escape_string(trim($product['name'])) . "',product_no='" . $product['code'] . "',image='" . $product['image'] . "',description='" . mysql_escape_string(trim($product['description'])) . "',thumb_image='" . $product['thumbnail'] . "',price='" . $product['price'] . "',category='" . strtolower($product['productType']) . "',type_id='1'");  //step 6
            }
    ...
    $message12 .="a) Count of local static data untouched: " . static_product_active_cnt() . "<br>";
    $message12 .="b) Count of local static data de-activated: " . static_product_deactive_cnt() . "<br>";
    $message12 .="c) Count of local static deleted data inserted: " . static_add_cnt() . "<br>";
    $message12 .="d) Count of local dynamic deleted data inserted: " . dynamic_add_cnt() . "<br>";
    $message12 .="e) Count of local static [Deleted] data: " . static_delete_cnt() . "<br>";
    $message12 .="f) Count of local dynamic [Deleted] data: " . dynamic_delete_cnt() . "<br><br>";
    
    $message12 .="Moving on to Step 4" . date('Y-m-d H:i:s') . "<br><br>";
    $message12 .="STEP:4 [UPDATE LOCAL STATIC DATA, INSERT ADDED DATA]" . "<br><br>";
    $message12 .="a) Count of local API dynamic data: " . static_product_cnt() . "<br>";
    $message12 .="b) Count of local static added data inserted: " . static_add_cnt() . "<br>";
    $message12 .="c) Count of local dynamic added data inserted: " . dynamic_add_cnt() . "<br>";
    $message12 .="d) Count from local static [Deleted] data: " . static_delete_cnt() . "<br>";
    $message12 .="e) Count from local dynamic [Deleted] data: " . dynamic_delete_cnt() . "<br><br>";
    ...
    $message12 .="a) TOTAL RECORDS DELETED TODAY: " . dynamic_delete_cnt() . "<br>";
    ...
    $message12 .="d) TOTAL RECORDS ADDED TODAY: " . dynamic_add_cnt() . "<br>";
    ...
    $result = mysql_query("SELECT COUNT(id) `present_totalrow` FROM " . TABLE_PRODUCT); //Count from local API dynamic data
    $row = mysql_fetch_array($result);
    $present_totalrow = $row['present_totalrow'];
    mysql_free_result($result);
    ...
    $message12 .="b) Count from local API dynamic data: " . $present_totalrow . "<br>";
    ...
    $message12 .="h) Mismatched volume since last Cron [a - b] = " . ($i - $present_totalrow) . "<br>";
    $message12 .="i) Mismatched volume since last Cron [a - c] = " . ($i - $present_totalrow) . "<br><br>";
    ...
    echo $message = htmlentities($message . $message1 . $message12);
