...
h=e+w+c,q=e+y+c,k=a.find(".title").eq(c)._setId(q);a.find(".steps a").eq(c)._setId(f)._aria("controls",h).attr("href","#"+q).html(F(b.titleTemplate,{index:c+1,title:k.html()}));a.find(".body").eq(c)._setId(h)._aria("labelledby",q)}}function W(a,b,d){var c=v(G,b.stepsOrientation)===G.vertical?" vertical":"horizontal",e=f(n('<{0} class="{1}">{2}</{0}>',b.contentContainerTag,"content "+b.clearFixCssClass,a.html())),g=f(n('<{0} class="{1}">{2}</{0}>',b.stepsContainerTag,"steps "+b.clearFixCssClass,'<ul role="tablist"></ul>')),
