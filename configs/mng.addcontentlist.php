	...
	<script language="javascript">
		...
		function ValidProd()
			...
			function ValidProd()
			{
				...
				{
					txt+="  	contents  should not be empty.\n"
				}
				if(txt)
				{
					alert("Sorry!! you left some mandatory fields :\n\n"+ txt +"\n     Please Check");
			...
			function ValidProd()
			{
				...
				{
					txt+="  	status  should not be empty.\n"
				}
				if(txt)
				{
					alert("Sorry!! you left some mandatory fields :\n\n"+ txt +"\n     Please Check");
			...
			function ValidProd()
			{
				...
				{
					txt+="  	site title  should not be empty.\n"
				}
				if(txt)
				{
					alert("Sorry!! you left some mandatory fields :\n\n"+ txt +"\n     Please Check");
			...
			{
				var txt="";
				if(document.getElementById('meta_keys').value=='')
				{
					txt+="  	meta keys should not be empty.\n"
				}
				if(txt)
				{
					alert("Sorry!! you left some mandatory fields :\n\n"+ txt +"\n     Please Check");
			...
			function ValidProd()
			{
				...
				{
					txt+="  	meta title should not be empty.\n"
				}
				if(txt)
				{
					alert("Sorry!! you left some mandatory fields :\n\n"+ txt +"\n     Please Check");
			...
			{
				var txt="";
				if(document.getElementById('desc_type').value=='')
				{
					txt+="  	 desc type should not be empty.\n"
				}
				if(txt)
				{
					alert("Sorry!! you left some mandatory fields :\n\n"+ txt +"\n     Please Check");
...
$contents_desc=htmlentities($_REQUEST[contents]);
	if(!$_REQUEST['cont_id'])
	{
	    ...
	    $insert_query = "insert into " .$prev."contents set 
...
?>
	...
	<tr bgcolor=<?=$light?>><td height="32" class="header"  style='border-bottom:solid 1px #333333'><a href='couisine.list.php?addons_id=<?=$d['addons_id']?>&menuid=<?=$_GET[menuid]?>&menupid=<?=$_GET[menupid]?>' class="header" target="_parent">
...
?>
	...
	<tr bgcolor="#ffffff" class="lnk"> <td bgcolor="#e9e9e9"><b>Status :</b></td><td align="left"><input type="radio" name="cuisine_status"  checked="checked" value="Y" <? if($d["cuisine_status"]=="Y"){ echo" checked";}?> >Active <input type="radio" name="cuisine_status" value="N" <? if($d["cuisine_status"]=="N"){echo" checked";}?>> In Active</td></tr>
	...
	<tr bgcolor="#ffffff" class="lnk"> <td bgcolor="#e9e9e9"><b>Desc Type :</b></td><td align="left"><input type="radio" name="desc_type"  checked="checked" value="1" <? if($d["desc_type"]=="1"){ echo" checked";}?> >Footer <input type="radio" name="desc_type" value="N" <? if($d["desc_type"]=="2"){echo" checked";}?>> Customer Register</td></tr>
