<?php
mail('surajit@scriptgiant.com','test0','test0','test0');
session_start();
include "init.php";  // this is optional but useful for setting up database access constants etc
//$rescc = mysql_fetch_array(mysql_query("select * from ".$prev."paypal_settings where 1"));

//$myemail = $rescc['admin_mail']; 
$myemail = "surajit@scriptgiant.com"; 
...
mail('surajit@scriptgiant.com','test00','test00','test00');
...
if (!$fp) {
mail('surajit@scriptgiant.com','test1','test1','test1');
...
{
mail('surajit@scriptgiant.com','test2','test2','test2');
fputs ($fp, $header . $req);
while (!feof($fp)) {
$res = fgets ($fp, 1024);
if (strcmp ($res, "VERIFIED") == 0) {
mail('surajit@scriptgiant.com','test3','test3','test3');
	mail("surajit@scriptgiant.com","paypal1", $fp.',,,,,,,,'.$res,$header . $req);
...
// you must check at the least the following...
	  ...
	  mail('surajit@scriptgiant.com','test4','test4','test4');
	 //echo $order_id=base64_decode($_REQUEST[payment_id]);
	  mysql_query("UPDATE `cp_customer_order` SET `payment_status` = 'Paid' WHERE `order_id` = '$order_id'");
	  mail("surajit@scriptgiant.com","paypal1",',,,,,,,,',$header );
...
//
mail('surajit@scriptgiant.com','test5','test5','test5');
      $mail_To = $myemail;
